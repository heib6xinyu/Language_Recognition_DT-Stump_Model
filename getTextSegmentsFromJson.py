...
def extract_segments(input_json, language, output_10, output_20, output_50):
    ...
    for abstract in abstracts:
        ...
        if len(words) >= 50:
            ...
            segments_50.append(f"{language}: {segment}")
        elif len(words) >= 20:
            segment = ' '.join(words[:20])
            segments_20.append(f"{language}: {segment}")
        elif len(words) >= 10:
            segment = ' '.join(words[:10])
            segments_10.append(f"{language}: {segment}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("language", help="Language of the abstracts")
    ...
    extract_segments(args.input_json, args.language, args.output_10, args.output_20, args.output_50)
